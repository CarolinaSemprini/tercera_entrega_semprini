...
class CartsController {
  ...
  //aqui se realizo un cambio en cach
  async readById(req, res) {
    try {
      ...
      logger.error(`Error al buscar el carrito: ${error.message}`);
  ...
  /*async readByRender(req, res) {
    try {
      ...
      res.status(200).render("carts", { plainCart, cartId, title, first_name, role, email });
    } catch (error) {
  ...
  async readByRender(req, res) {
    try {
      ...
      const cart = await cartService.readByRender(cartId);
      if (!cart) {
        return res.status(404).json({ message: "No se encontró el carrito asociado al usuario" });
      }
      const title = "Productos en Carrito";
      const { first_name, role, email } = req.session.user;
      const plainCart = cart.products.map(doc => doc.toObject());
