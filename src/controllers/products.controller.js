...
class ProductsController {
  ...
  async readByRenderUser(req, res) {
    try {
      const { limit, pagina, category, orderBy } = req.query;
      const data = await productService.readWithPagination(limit, pagina, category, orderBy);
      const { first_name, role, cartId, email, owner } = req.session.user;
  ...
  async readByRenderAdmin(req, res) {
    try {
      ...
      if (userRol === "user") {
        ...
        return res.status(200).render("products-admin", { products, title, first_name, userRol });
      }
  ...
  async readByIds(req, res) {
    try {
      const { ids } = req.body; // Suponiendo que los IDs están en el cuerpo de la solicitud
      const productsByIds = await productService.readByIds(ids);
      if (productsByIds) {
        return res.status(201).json({
          status: "success",
          msg: `Mostrando los productos con los IDs proporcionados`,
          payload: { productsByIds },
        });
      } else {
        logger.error(`No se encontraron productos con los IDs proporcionados`);
        return res.status(201).json({
          status: "success",
          msg: `No se encontraron productos con los IDs proporcionados`,
        });
      }
    } catch (e) {
      logger.error(e.message);
